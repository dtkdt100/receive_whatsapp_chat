          ...
          children: [
            ...
            Column(
              children: List.generate(3, (index2) {
                MessageContent randomMessage = chats[index]
                    .messages[Random().nextInt(chats[index].messages.length)];
                return Padding(
                  padding: const EdgeInsets.only(left: 20),
                  child: Column(
  ...
  Widget buildFutureImageBuilder(Future<ui.Image?> img) => FutureBuilder(
    ...
    builder: (BuildContext context, AsyncSnapshot<ui.Image?> image) {
      if (image.hasData && image.data != null) {
        return RawImage(image: image.data, scale: 4.5); // image is ready
      } else {
